...
var (
	...
	typeOfContx1_8 = reflect.TypeOf((*contx.Context)(nil)).Elem()
)
...
type serviceMethod struct {
	...
	contextType reflect.Type   // type of the context
...
func (m *serviceMap) register(rcvr interface{}, name string) error {
	...
	for i := 0; i < s.rcvrType.NumMethod(); i++ {
		...
		// Method needs four ins: receiver, context, *req.
		if mtype.NumIn() != 3 {
			continue
		}
		...
		if contextType.Kind() != reflect.Interface || contextType != typeOfContext {
			switch contextType {
			case typeOfContext:
			case typeOfContx1_8:
				break
		...
		// Next argument must be a pointer and must be exported.
		if resType := mtype.Out(0); resType.Kind() != reflect.Ptr || !isExportedOrBuiltin(resType) {
			continue
		}
